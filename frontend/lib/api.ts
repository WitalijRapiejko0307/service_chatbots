...
export const api = {
  ...
  async getChannelSupportMatrix(): Promise<import("./types/channel").ChannelSupportMatrix> {
    return request<import("./types/channel").ChannelSupportMatrix>(
      "/api/v1/admin/channel-support-matrix",
      {},
      true
    );
  },

  async startInstagramOAuth(agentId: string): Promise<{ authorization_url: string }> {
    return request<{ authorization_url: string }>(
      `/api/v1/instagram/oauth/start?agent_id=${encodeURIComponent(agentId)}`,
      { headers: { Accept: "application/json" } },
      true
    );
  },

  async startTikTokOAuth(agentId: string): Promise<{ authorization_url: string }> {
    return request<{ authorization_url: string }>(
      `/api/v1/tiktok/oauth/start?agent_id=${encodeURIComponent(agentId)}`,
      { headers: { Accept: "application/json" } },
      true
    );
  },
