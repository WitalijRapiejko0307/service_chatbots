...
/**
 * Get company display name (service business).
 */
export function getCompanyDisplayName(agent: Agent | null | undefined): string | null {
  if (!agent?.config?.profile?.company_display_name) {
    return null;
  }
  return agent.config.profile.company_display_name;
...
/** @deprecated Use getCompanyDisplayName — kept for copied CRM call sites. */
export function getClinicDisplayName(agent: Agent | null | undefined): string | null {
  return getCompanyDisplayName(agent);
}
