...
export function InstagramChannelGuide({
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted">{t("instagram.intro")}</p>
      <div className="text-xs text-foreground bg-surface-hover/60 rounded-sm px-3 py-2">
      ...
      <div className="space-y-3">
        ...
        <Step n={3}>
          ...
          <div className="text-xs text-muted mt-0.5 mb-2">{t("instagram.step3Help")}</div>
          <div className="space-y-1 mb-3">
            <div className="text-xs font-medium text-foreground">
              {t("instagram.callbackUrl")}{" "}
              <span className="font-normal text-muted">{t("instagram.callbackUrlHint")}</span>
            </div>
          ...
          <div className="space-y-1">
            ...
            {config?.instagram_verify_token ? (
              <>
                ...
                <div className="text-xs text-muted">{t("instagram.verifyTokenSameValue")}</div>
              </>
            ) : (
              <InlineTokenSetup
                channel="instagram"
          ...
          <div className="text-xs text-muted mt-3 bg-surface-hover/60 rounded-sm px-3 py-2">
            {t.rich("instagram.verifyAfterSave", richMarks())}
          </div>
        </Step>
        <Step n={4}>{t.rich("instagram.step4", richMarks())}</Step>
        <Step n={5}>{t("instagram.step5")}</Step>
        <p className="text-xs text-muted">{t("instagram.oauthPathNote")}</p>
        <p className="text-xs text-muted">{t("instagram.tokenExpiryNote")}</p>
      </div>
    </div>
  );
}
