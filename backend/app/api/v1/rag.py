...
def _ensure_cloudinary_storage() -> None:
    raise HTTPException(
        ...
        detail="Cloudinary import is not available in Service ChatBot. Upload files directly.",
    )
...
class CloudinaryImportRequest(BaseModel):
    items: list[CloudinaryImportItem] = Field(..., min_length=1, max_length=50)
...
async def list_cloudinary_rag_resources(
    ...
    """Search Cloudinary assets — not available in Service ChatBot."""
...
async def import_rag_documents_from_cloudinary(
    ...
    """Import from Cloudinary — not available in Service ChatBot."""
    _ensure_cloudinary_storage()
