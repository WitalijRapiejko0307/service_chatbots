...
def require_production_encryption_key(settings: Settings) -> None:
    """Fail fast in production if SECRET_ENCRYPTION_KEY is missing or invalid.

    Development remains optional until secrets are written.
    """
    if str(getattr(settings, "environment", "") or "").lower() != "production":
        return
    _get_fernet(settings)
